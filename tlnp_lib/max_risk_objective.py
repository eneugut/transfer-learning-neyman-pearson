...
class MaxRiskObjective(nn.Module):
    def __init__(
        ...
        # Three multipliers (one per constraint)
        self.register_buffer("lmbda1", torch.tensor(0.0), persistent=False)  # for f1
        self.register_buffer("lmbda2", torch.tensor(0.0), persistent=False)  # for f2
        self.register_buffer("lmbda3", torch.tensor(0.0), persistent=False)  # for f3
        # α′ (projected to ≥ α in the trainer)
        self.alpha_prime = nn.Parameter(torch.tensor(float(alpha), dtype=torch.float32))
    ...
    def _loss_on_subset(logits_subset: torch.Tensor, target01: float, loss_function_type: str) -> torch.Tensor:
        if loss_function_type == "LogisticLoss":
        ...
        elif loss_function_type == "ExponentialLoss":
            y_tilde = (2.0 * float(target01) - 1.0)   # 0->-1, 1->+1
            margin = -y_tilde * logits_subset
            margin_clamped = torch.clamp(margin, max=20.0)   # numeric safety
    ...
    def compute_Rs(self, X, labels):
        ...
        R0T = self._loss_on_subset(logits[T0_mask], target01=0.0, loss_function_type=self.loss_function_type)
    ...
    def f_values(self, R0T, R1T, R0S):
        f1 = R0T - (self.alpha + self.epsilon_0_T)                    # g1
        f2 = R0S - (self.alpha_prime + self.epsilon_0_S)              # g2 (depends on α′)
        f3 = R1T - (self.R1T_hat_const + self.epsilon_1_T)            # g3
        return f1, f2, f3
    ...
    def forward(self, X, labels):
        ...
        g_max, idx = torch.max(f_stack, dim=0)

        # Lagrangian (exactly as theory: α′ + λ1 f1 + λ2 f2 + λ3 f3)
        L = self.alpha_prime + self.lmbda1 * f1 + self.lmbda2 * f2 + self.lmbda3 * f3
        return L, g_max, (f1, f2, f3), (R0T, R1T, R0S)
